...
| Testing tools | Static validation, unit tests, smoke checks | `package.json`, `tests/*`, `scripts/*`, `.github/workflows/ci.yml` | None | Medium | Vitest unit tests exist for helpers, route wrappers, Stripe webhook helpers, and atomic invoice RPC flag behavior; E2E, preview Stripe replay, and RLS tests still needed. |
