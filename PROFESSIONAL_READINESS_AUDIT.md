...
- Current estimated completion: 83% beta readiness; 58% production readiness.
...
| Testing Coverage | 2.4/4 | Needs Work | Vitest unit tests for billing/env/invoice/security/Stripe webhook/billing route guardrails plus a signed webhook route fixture | Add authenticated E2E/API/RLS tests and preview Stripe replay. |

Current beta-readiness percentage: 83%. Current production-readiness percentage: 58%. Confidence: high for local code/build/test findings; medium for live Supabase/Stripe state because hosted database and live Stripe are not fully proven in this pass.
...
6. No automated auth/RLS/E2E tests; unit tests now cover pure helper, billing route helper, Stripe webhook helper, signed webhook route fixture, and guardrail logic only.
...
Vitest unit tests now cover pure billing/env helpers, billing route response/session builders, invoice validation, request-security helpers, Stripe webhook processing helpers, a signed Stripe webhook route fixture, and the static readiness guard. Smoke and readiness scripts cover public routes, protected redirects, selected unauthenticated API behavior, blocked stale copy, required files, security headers, and saved-only PDF export guardrails. Missing: authenticated browser E2E, broader API integration tests with mocked Supabase/Stripe, Supabase RLS/cross-user tests, accessibility tests, visual regression tests, and Stripe CLI replay on preview/live. See `QA_TEST_PLAN.md`.
...
This site is not beta-ready today until the current working tree is pushed/deployed to preview and authenticated QA passes. It is not production-ready. Current score: 83/100 beta readiness, 58/100 production readiness.
